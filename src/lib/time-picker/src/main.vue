...
import {
  isArray, isDate, isString, deepCopy,
} from '../../../utils/assist';
...
export default {
  ...
  mixins: [Emitter],
  components: {
    MkuInput, MkuDrop, MkuButton, MkuPicker,
  },
  props: {
    // 绑定的值
    value: {
  ...
  methods: {
    ...
    handleInputEnter(event, val) {
      const nums = this.pickNumberInString(val);
    ...
    handlePickerClick(type, arg) {
      ...
      if (type === 'start') {
        ...
        // 结束时间项不能比开始时间小
        const start = fill(deepCopy(this.startValue));
        const end = fill(deepCopy(this.endValue));
        end[RowColIdx] = arg[1];
    ...
    getEmitValue() {
      const getValue = (val) => this.formatOutputValue(val, this.formatValue);
      return this.isRange ? [
        getValue(this.startValue),
        getValue(this.endValue),
      ] : getValue(this.startValue);
    },
    /**
    ...
    handleInputClick(event) {
      ...
      if (isClearBtn) this.handleClear();
      else this.handleToggleTpicker(true);
